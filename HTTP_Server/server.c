...
void serve_file(int socket_fd, char *path) {
  ...
  http_send_header(socket_fd, "Content-Type", http_get_mime_type(path));
  FILE* f = fopen(path,"rb");
  char size[100];
  fseek(f, 0, SEEK_END);
  int len = ftell(f);
  snprintf (size, 100, "%d", len);
  http_send_header(socket_fd, "Content-Length", size);
  http_end_headers(socket_fd);
  char buffer[len+1];
  fseek(f, 0, SEEK_SET);
  fread(buffer, len, 1, f);
  fclose(f);
  http_send_data(socket_fd,buffer,len);
}
...
void serve_directory(int socket_fd, char *path) {
  char *add =  "/index.html";
  char *index = malloc(strlen(path) + strlen(add) + 1);
  memcpy(index, path, strlen(path));
  memcpy(index + strlen(path), add, strlen(add)+1);
  struct stat file_stat;
  if(stat(index, &file_stat)==0){
    serve_file(socket_fd,index);
    return;
  } else {
    http_start_response(socket_fd, 200);
    ...
    http_end_headers(socket_fd);
    char data[102400];
    sprintf(data,"<center><h1>%s</h1></center>\n<ul>\n<li><a href=\"../\">Parent directory</a></li>\n",path);
    ...
    do {
        errno = 0;
        if ((dp = readdir(dirp)) != NULL && strcmp(dp->d_name,"..")!=0 && strcmp(dp->d_name,".")!=0) {
          char buffer[1024];
    ...
    closedir(dirp);
    strcat(data,"</ul>");
    http_send_string(socket_fd, data);
    return;
  }
}
